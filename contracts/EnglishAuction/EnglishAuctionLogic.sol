 ...
 * @author  thirdweb.com
 */
abstract contract EnglishAuctionLogic is IEnglishAuctions, ReentrancyGuardLogic, ERC2771ContextConsumer {
    /*///////////////////////////////////////////////////////////////
    ...
    /// @dev Only notary role holders can finalize auctions.
    bytes32 public constant NOTARY_ROLE = keccak256("NOTARY_ROLE");
    /// @dev Only inspector role holders can inspect properties.
    ...
    mapping(uint256 => bool) private inspections;
    ...
    function updateInspectionStatus(uint256 _propertyID, bool _status) public onlyInspectorRole {
    ...
    {
        // the property has to be inspected before it can be auctioned
        require(inspections[_params.tokenId], "Marketplace: Property has not been inspected.");
    ...
    function _collectAuctionPayout(uint256 _auctionId) internal nonReentrant {
     ...
     // function to approve the auction winner and transfer the tokens to the winner from notary
    function approveWinner(uint256 _auctionId) external onlyNotaryRole {
        EnglishAuctionsStorage.Data storage data = EnglishAuctionsStorage.englishAuctionsStorage();
        require(data.auctions[_auctionId].status == SaleStatus.WAITING_FOR_APPROVAL, "Marketplace: Not waiting for notary approval.");
        data.auctions[_auctionId].status = SaleStatus.APPROVED;
        _collectAuctionTokens(_auctionId);
        _collectAuctionPayout(_auctionId);
    }

    function _collectAuctionTokens(uint256 _auctionId) internal nonReentrant {
        EnglishAuctionsStorage.Data storage data = EnglishAuctionsStorage.englishAuctionsStorage();
        Auction memory _targetAuction = data.auctions[_auctionId];
        Bid memory _winningBid = data.winningBid[_auctionId];
    ...
    function _handleBid(Auction memory _targetAuction, Bid memory _incomingBid) internal {
        ...
        // update auction and execute saleStatus change if there's a buyout price and incoming bid amount is buyout price.
        if (_targetAuction.buyoutBidAmount > 0 && incomingBidAmount >= _targetAuction.buyoutBidAmount) {
            ...
            data.auctions[_targetAuction.auctionId].status = SaleStatus.WAITING_FOR_APPROVAL;
        } else {
            /**
